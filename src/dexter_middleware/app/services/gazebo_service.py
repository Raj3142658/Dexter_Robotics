...
class GazeboService:
    ...
    def _tail_logs(self, max_lines: int = 40) -> str:
        if not self._log_path:
            return "(no gazebo log file)"
        try:
            with open(self._log_path, "r", encoding="utf-8", errors="replace") as f:
                lines = f.readlines()
            return "".join(lines[-max_lines:]).strip() or "(gazebo log file empty)"
        except Exception:
            return "(failed to read gazebo log file)"
    ...
    def _required_children_present(self) -> bool:
        ...
        for pid, _ppid, cmd in self._ps_rows():
            ...
            if "gz sim" in cmd_l or "gazebo" in cmd_l:
                has_gazebo = True
            if "gz sim" in cmd_l or "rviz2" in cmd_l:
                has_gui = True
        return has_gazebo and has_gui

    def status(self) -> GazeboStatus:
    ...
    def start(self, gui: bool = True) -> GazeboStatus:
        current = self.status()
        if current.running:
            return current
        ...
        self._expect_gui = gui

        try:
            log_file = tempfile.NamedTemporaryFile(
                mode="w",
                prefix="dexter_gazebo_",
                suffix=".log",
                delete=False,
                encoding="utf-8",
            )
            ...
            raise RuntimeError(
                "Unable to start Gazebo: 'ros2' command not found in middleware environment. "
        ...
        if self._process.poll() is not None:
            ...
            self._started_at = None
            raise RuntimeError(
                "Gazebo launch exited immediately. Ensure install/setup.bash is sourced and gazebo dependencies are installed. "
                f"Recent logs:\n{logs}"
            )
        ...
        while time.time() < deadline:
            ...
            if self._required_children_present():
                ...
                elif (time.time() - stable_seen_at) >= 2.8:
                    return self.status()
            else:
                stable_seen_at = None
            time.sleep(0.2)
        ...
        self.stop()
        raise RuntimeError(
            "Gazebo required processes did not stay up after launch. "
            f"Recent logs:\n{logs}"
        )
